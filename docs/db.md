...
From a bird's eye view, the database facilitates the data operations of a made-up bank. Right now, the bank services are limited: customers make a one-time deposit of 1000 into an account when they open it and customers can transfer money but only to other customers within the same bank. Additionally, customers can check their balance, last 5 transactions and full transactions history.

The three main tables underlying the database are: `customer`, `account` and `transfer_log`. There is a one-to-many relationship from `customer` to `account`, that is, customers can have zero, one or more accounts with the bank. There is also a one-to-many relationship from `account` to `transfer_log` entries: each transfer log entry is associated with a sender's account and the receiver's account.



## customer table

The customer table is pretty standard and self-explanatory. Note, that the database does not carry out encryption of the password column, this is to be carried out at the application layer.
...
One of the business rules of the bank is that account balances should not go below 0 and this is enforced via the constraint `balance_nonnegative`. An additional business rule is that there are no 'dangling' accounts, each account must be linked to a `customer`. Therefore, the `customer_id` is constrained to non-null values. Furthermore, the foreign-key constraint ensures that a `customer` entry cannot be deleted as long as an account has been registered under its name and accounts can not be transfered from one customer to another. Additionally, accounts themselves cannot be deleted, they can only be deactivated. This is enforced with the following trigger:
